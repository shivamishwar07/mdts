...
//   const [privacyPost, setPrivacyPost] = useState<any | null>(null);
//   const [privacyVisibility, setPrivacyVisibility] = useState<ShareVisibility>("private");
//   const [privacySharedWith, setPrivacySharedWith] = useState<string[]>([]);
...
//   const [personQuery, _setPersonQuery] = useState("");
//   const [personFilter, _setPersonFilter] = useState<string | null>(null);
...
//       try {
//         const myEmail = getCurrentUserEmail();
//         const ps = await db.getKnowledgePostsForUser(myEmail);
//         setPosts(ps || []);
//         setLikedByMe(Object.fromEntries((ps || []).map((p: any) => [Number(p.id), !!p.myLiked])));
//       } catch {
//         setPosts([]);
//       }
//     })();
...
//     const list = posts.filter(canView);
//     const myEmail = getCurrentUserEmail();

//     if (feedFilter === "mine") {
//       return list.filter((p: any) => String(p?.createdBy?.email || "").toLowerCase() === myEmail);
//     }
//     return list;
//   }, [posts, feedFilter]);
...
//   const allCount = useMemo(() => posts.filter(canView).length, [posts]);
//   const mineCount = useMemo(
//     () =>
//       posts
//         .filter(canView)
//         .filter((p: any) => String(p?.createdBy?.email || "").toLowerCase() === myEmail).length,
//     [posts, myEmail]
//   );
...
//     const q = norm(postQuery);

//     let list = visiblePosts;

//     if (personFilter) {
//       list = list.filter((p: any) => getAuthorKey(p) === personFilter);
//     } else {
//       const pq = norm(personQuery);
//       if (pq) {
//         list = list.filter((p: any) => {
//           const authorText = norm(getAuthorLabel(p));
//           return authorText.includes(pq);
//         });
//       }
//     }
...
//       const authorName = norm(p?.createdBy?.name);
//       const authorEmail = norm(p?.createdBy?.email);

//       return title.includes(q) || body.includes(q) || authorName.includes(q) || authorEmail.includes(q);
//     });
//   }, [visiblePosts, postQuery, personQuery, personFilter]);
...
//   const openPrivacyModal = (p: any) => {
//     setPrivacyPost(p);
//     setPrivacyVisibility((p.visibility as ShareVisibility) || "private");
//     setPrivacySharedWith(
...
//     if (!privacyPost?.id) return;

//     if (privacyVisibility === "restricted" && privacySharedWith.length === 0) {
...
//       await db.knowledgePosts.update(Number(privacyPost.id), {
...
//       notification.success({ message: "Privacy updated" });
//       closePrivacyModal();
//       await reloadPosts();
//     } catch (e: any) {
//       notification.error({ message: e?.message || "Failed to update privacy" });
//     } finally {
...
//     try {
//       const dataUrl = await db.getDiskEntry(att.path);
//       if (!dataUrl) {
//         notification.error({ message: "File not found in IndexedDB" });
//         return;
//       }
...
//     } catch (e: any) {
//       notification.error({ message: e?.message || "Failed to download file" });
//     }
//   };
...
//   function getAuthorKey(p: any) {
//     return norm(p?.createdBy?.email || p?.createdBy?.name || "");
//   }

//   function getAuthorLabel(p: any) {
//     const name = String(p?.createdBy?.name || "").trim();
//     const email = String(p?.createdBy?.email || "").trim();
...
//                 <Space size={10}>
//                   <Tooltip title="Filter what you see in the feed">
//                     <Segmented
//                       className="ks-segment"
//                       value={feedFilter}
//                       onChange={(v) => setFeedFilter(v as any)}
//                       options={[
//                         {
//                           label: (
//                             <span className="ks-seg-item">
//                               <AppstoreOutlined />
//                               <span>All</span>
//                               <span className="ks-seg-count">{allCount}</span>
//                             </span>
//                           ),
//                           value: "all",
//                         },
...
//                               <UserOutlined />
//                               <span>My Posts</span>
//                               <span className="ks-seg-count">{mineCount}</span>
//                             </span>
//                           ),
//                           value: "mine",
//                         },
...
//                     const canOwner = canEditOrDelete(p);
...
//                               <Button
//                                 size="small"
//                                 className={`ks-eng-btn ${myLiked ? "reacted" : ""}`}
//                                 onClick={async () => {
...
//         width={760}
//         className="ks-modal modal-container"
//       >
...
//             <Select className="ks-select" value={privacyVisibility} onChange={(v) => setPrivacyVisibility(v)}>
...
//                 value={privacySharedWith}
//                 onChange={setPrivacySharedWith}
//                 options={users
...
//         width="80%"
//         title="Preview"
//         className="ks-preview-modal modal-container"
//       >
...
//         cancelText="Cancel"
//         width={640}
//         className="ks-modal ks-delete-modal modal-container"
//       >
//         <Alert
//           type="warning"
//           showIcon
...
//           <div className="ks-delete-title">{deletingPost?.title || "Untitled Post"}</div>

//           <div className="ks-delete-meta">
//             <span>
//               Author: <b>{deletingPost?.createdBy?.name || deletingPost?.createdBy?.email || "Unknown"}</b>
//             </span>
//             <span className="ks-dot">•</span>
//             <span>
